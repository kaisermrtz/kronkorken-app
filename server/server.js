...
var {
  ...
  getSimilarDocuments,
  importModel,
} = require('./util/contentBasedRecommender')
...
app.get('/', async (req, res) => {
  ...
  var recentlyAdded
  try {
    var crowncaps = await CrownCap.find({}).sort({
      addedAt: -1,
    })
    ...
    var brandCountArray = await CrownCap.aggregate([
      {
        $group: {
          _id: '$brand',
          count: {
    ...
    var typeCountArray = await CrownCap.aggregate([
      {
        $group: {
          _id: '$typeOfDrink',
          count: {
    ...
    var randomCrownCaps = await CrownCap.aggregate([
      {
        $sample: {
          size: 6,
        },
    ...
    var historyCountArray = await CrownCap.aggregate([historyGroup]).sort({
      '_id.year': 1,
      '_id.month': 1,
    })
  ...
  res.render('home.hbs', {
    ...
    count,
    countries,
    brandCount: JSON.stringify(brandCountArray.slice(0, 5)).replace(
      new RegExp("'", 'g'),
      ''
    ),
...
app.get('/sammlung', async (req, res) => {
  ...
  if (req.query.typeOfDrink && req.query.typeOfDrink !== '') {
    optionsArray.push({
      typeOfDrink: new RegExp(req.query.typeOfDrink, 'i'),
    })
  }
...
app.get('/new-trade', async (req, res) => {
  try {
    var crowncapsDE = await CrownCap.find({
      quantity: {
        $gt: 0,
      },
      country: 'Deutschland',
    ...
    var crowncapsNotDE = await CrownCap.find({
      quantity: {
        $gt: 0,
      },
      country: {
    ...
    var crowncapsSpecial = await CrownCap.find({
      quantity: {
        $gt: 0,
      },
      special: true,
...
app.get('/sammlung/:id', async (req, res) => {
  ...
  try {
    ...
    if (similarDocuments.length == 0) {
      ...
      if (similarCrownCaps.length < 6) {
        var randomCrownCaps = await CrownCap.aggregate([
          {
            $sample: {
              size: 6 - similarCrownCaps.length,
            },
          },
        ])
    ...
    if (crowncap._tradeTransaction) {
      var transaction = await TradeTransaction.findById(
        crowncap._tradeTransaction
      )
...
app.post('/sammlung/:id/edit', requiresLogin, async (req, res) => {
  try {
    if (req.body.oldCloudinaryImageId === req.body.cloudinaryImageId) {
      var crownCapData = _.pick(req.body, [
        'name',
        'brand',
        'country',
        'typeOfDrink',
      ...
      //Altes Bild löschen
      var returnValue = await cloudinaryAsyncDelete(
        req.body.oldCloudinaryImageId
      )
    }
...
app.post('/sammlung/:id/delete', requiresLogin, async (req, res) => {
  ...
  try {
    ...
    var crowncap = await CrownCap.findOneAndRemove({
      _id: id,
    })
    if (!crowncap) {
      return res.redirect(`/sammlung/${req.body.id}/edit/?success=false`)
...
app.get('/dashboard', requiresLogin, async (req, res) => {
  ...
  res.render('dashboard.hbs', {
    ...
    brandCountArray: JSON.stringify(brandCountArray.slice(0, 30)).replace(
      new RegExp("'", 'g'),
      ''
    ),
  })
...
app.get('/history', async (req, res) => {
  try {
    ...
    for (var i = 0; i < transactions.length; i++) {
      ...
      var crowncaps = await CrownCap.find({
        _tradeTransaction: transactions[i]._id,
      })
...
app.post('/history/delete/transaction', requiresLogin, async (req, res) => {
  ...
  try {
    ...
    var transaction = await TradeTransaction.findOneAndRemove({
      _id: id,
    })
...
app.post('/history/delete/partner', requiresLogin, async (req, res) => {
  ...
  try {
    var partner = await TradePartner.findOneAndRemove({
      _id: id,
    })
    if (!partner) {
      return res.redirect(`/history?success=false`)
    }
    var transactions = await TradeTransaction.find({
      _tradePartner: id,
    })

    for (var j = 0; j < transactions.length; j++) {
      var crowncaps = await CrownCap.find({
        _tradeTransaction: transactions[j]._id,
      })
      for (var i = 0; i < crowncaps.length; i++) {
        var newCrownCap = await CrownCap.findOneAndUpdate(
          {
    ...
    transactions = await TradeTransaction.find({
      _tradePartner: id,
    }).remove()
    res.redirect('/history?success=true')
  } catch (e) {
